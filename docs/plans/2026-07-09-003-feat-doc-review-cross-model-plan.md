...
- Reuses ce-doc-review's existing missing-document gate — Phase 1 confirms the document is readable on disk before any dispatch; the orchestrator embeds that content in the peer prompt (KTD3, R13). The peer is tool-less, deny-Read, and runs in a scratch cwd — it never reads the repo filesystem.
