...
**Trust boundary:** the pass embeds the full document content into the peer prompt and sends it to an external model provider (OpenAI, Anthropic, xAI, or Cursor, depending on the resolved peer); `CROSS_MODEL_PEERS` restricts which providers may receive content (unset = default order; set = allowlist). The peer runs strictly read-only and **tool-less**, from an empty scratch dir with no project context — every route denies file reads, web, MCP, and subagents where the CLI supports it (codex `-s read-only -C <scratch>`; claude `--bare --tools ""` — all built-in tools disabled and no CLAUDE.md/MCP auto-discovery; grok denies `Read`/`Edit`/`Write`/`Bash`/`Task`/web/`mcp__*`; the cursor-agent routes run `--mode ask --sandbox enabled` in a scratch workspace) — so impact is bounded to disclosure rather than repo mutation, and the script emits a one-line audit log of each cross-model send so the egress is auditable even in headless mode. Peer prompts use basename-only document paths (content is already embedded). Over-size documents skip cleanly rather than truncating. A document from an untrusted author is a prompt-injection surface contained by the read-only, tool-less posture to disclosure-to-self. One accepted residual: `cursor-agent --mode ask` cannot fully disable its Read tool, a weaker isolation on the grok-fallback and Composer routes — accepted for the own-document threat model since the host agent already runs in-repo with more privilege than any peer.
