...
- **Non-thread feedback** (top-level PR comments + review-submission bodies). Surfaced as `actionable.comments` for feedback that has **no inline thread** — a Changes-Requested review summary or a bare top-level "please rename X" — from non-author, non-CI-bot accounts (code-review bots like Codex/CodeRabbit/Greptile *are* included; pure status bots like codecov/CI are filtered out). Unlike a thread there is **no remote resolve**, so such an item never drops out of the fetch on its own: `mark --comment <id> --disposition dispatched` (handled) or `--disposition needs-human` (escalated) is the *only* thing that silences it. Same open/dispatched/needs-human semantics and re-open (`--disposition open`) as threads. Both streams count as one **review** stream for the trajectory (a bot re-posting fresh top-level nits every commit is a treadmill, not silence) and for the merge-ready backlog (`counts.threads` **and** `counts.comments` must both be 0).
