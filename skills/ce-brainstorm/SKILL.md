...
Write to `docs/plans/YYYY-MM-DD-NNN-<type>-<topic>-plan.<md|html>` — extension follows `OUTPUT_FORMAT`. Include `artifact_contract: ce-unified-plan/v1`, `artifact_readiness: requirements-only`, and `product_contract_source: ce-brainstorm`. Title is `<Name> — Requirements` (matching the H1; no conventional-commit prefix) — not `- Plan`: `artifact_readiness: requirements-only` means this is not a plan yet, and the heading should say so. `ce-plan` renames the H1 to `- Plan` when its enrichment pass promotes the same file to `artifact_readiness: implementation-ready`. Keep the doc light and standalone-readable: a Goal Capsule (objective, product authority, open blockers) and the Product Contract. Do **not** emit a Goal Launch Block or Reader Index. See `references/brainstorm-sections.md` — which owns the artifact content rules, including repo-relative file paths inside the doc. When confirming in chat, report the written artifact with its absolute path so the reference is clickable.
