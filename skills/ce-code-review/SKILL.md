...
When `adversarial-reviewer` was selected (Stage 3) **and** scope is `local-aligned` or standalone, also run the same adversarial brief through **one different model provider than the host** in a separate read-only process — genuine independence the in-process subagent cannot provide. **Launch it in parallel with the persona reviewers, not after them:** the peer call is a CLI shell-out (not a subagent), so it does not consume the subagent concurrency budget and its ~2-5 min runtime overlaps the in-process reviews instead of adding to them. Kick off one `peer-job-runner.py start` call in the same dispatch wave as the Stage 4 reviewers — the runner detaches the worker into its own supervised session and prints a job id in seconds; never hold a Bash call open for the peer's runtime. Poll with bounded `wait --max-secs 30` calls interleaved with Stage 4/5 work; before Stage 5, loop bounded `wait` until the job is terminal or 610s from start, `reap` a still-nonterminal job at that deadline, then take one final bounded `wait --max-secs 10` (reap is asynchronous — the terminal record lands a grace period after it returns), then fold in the artifact if present and name a started-but-not-done peer in Coverage with its terminal state. Load `references/cross-model-review.md` and follow it: **attest the host provider** (Claude Code → `claude`; Codex → `codex`; Cursor → its active serving provider; un-attestable → skip, never guess), **resolve the peer preference** into a comma-separated candidate list (conversation → `.compound-engineering/config.local.yaml` `cross_model_peer:` → project instructions in context → default `codex,claude,grok,composer`), and invoke the bundled script with `host_provider`, `candidates`, Stage 1 `BASE`, and the run dir. The script owns availability probing, host exclusion, grok→cursor-agent fallback, and writing `adversarial-<provider>.json` only after normalize. This pass is **adversarial-only** — no other persona gets a cross-model twin.

That return enters Stage 5 as reviewer `adversarial-<provider>`, like any persona artifact. The pass is **non-blocking** — skip silently when it never starts (host un-attestable, no different provider reachable, peer CLI missing/unauthed); a started peer that fails, times out, dies, or is reaped still never blocks the review, but is named in Coverage with its terminal state rather than vanishing silently. Skip it entirely in `pr-remote` / `branch-remote` scope (the peer would review the local tree, not the reviewed head). Announce per that reference's announce rules — interactive hosts in default mode (with egress disclosure); silent in `mode:agent` (stderr audit log still records egress).
