...
- Set `artifact_contract: ce-unified-plan/v1`, `artifact_readiness: implementation-ready`, and `execution: code` for software implementation plans. If the source was a requirements-only unified plan whose H1 reads `<Name> — Requirements` (the `ce-brainstorm` title convention for that readiness stage), rename the H1 to `<Name> - Plan` at the same time — the heading should track `artifact_readiness`, not lag behind it.
